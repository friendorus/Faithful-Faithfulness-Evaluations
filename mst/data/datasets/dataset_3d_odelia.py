...
def parse_local_data(path_root):
    ...
    """
    path_root = Path(path_root) if not isinstance(
        path_root, Path) else path_root
    df = pd.read_csv(path_root / "metadata_unilateral" /
                     "annotation.csv", index_col=0, dtype={'PatientID': str})
...
class ODELIA_Dataset3D(data.Dataset):
    PATH_ROOT = Path(os.getenv("DATASET_PATH")).resolve()
    ...
    def __init__(
        ...
        self.path_root = self.PATH_ROOT if path_root is None else Path(
            path_root)
        ...
        if transform is None:
            self.transform = tio.Compose([
                tio.Resize(image_resize) if image_resize is not None else tio.Lambda(
                    lambda x: x),
                tio.Resample(resample) if resample is not None else tio.Lambda(
                    lambda x: x),
                tio.Flip(1),  # Just for viewing, otherwise upside down
                CropOrPad(image_crop, random_center=random_center,
                          padding_mode='minimum') if image_crop is not None else tio.Lambda(lambda x: x),
                ZNormalization(per_channel=True, per_slice=False, masking_method=lambda x: (
                    x > x.min()) & (x < x.max()), percentiles=(0.5, 99.5)),   # 0.5, 99.5   2.5, 97.5
                ...
                tio.RandomFlip((0, 1, 2)) if flip else tio.Lambda(
                    lambda x: x),  # WARNING: Padding mask
                tio.Lambda(lambda x: -x if torch.rand((1,),)[0] < 0.5 else x, types_to_apply=[
                           tio.INTENSITY]) if noise else tio.Lambda(lambda x: x),
                tio.RandomNoise(std=(0.0, 0.25)) if noise else tio.Lambda(
                    lambda x: x),
        ...
        if item_pointers is not None:
            ...
            self.item_pointers = [
                p for p in item_pointers if p in self.df.index]
        else:
    ...
    def __getitem__(self, index):
        ...
        path_item = [self.path_root_data/uid /
                     f'{sequence}.nii.gz' for sequence in self.sequence]
        if len(path_item) == 1:
            img = self.load_img(path_item[0])
        else:
            # Load multiple sequences and stack them
            imgs = [self.load_img(p) for p in path_item]
            # For now, use first sequence; can be extended to stack
            img = imgs[0]
