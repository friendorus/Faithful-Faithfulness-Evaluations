...
#----- Prediction wrapper for Transformer models -----
def _pred_transformer(model, source, src_key_padding_mask, save_attn=False, use_softmax=True):
    '''Run prediction for transformer-based models 
    [Possible to use ResNetSliceTrans or DinoV2ClassifierSlice]
    options to save attention maps and use softmax
    
    Parameters:
    model                   : a PyTorch model (possibly ResNetSliceTrans)
    source                  : input tensor (likely [B, C, D, H, W])
    src_key_padding_mask    : mask for padded tokens/slices
    save_attn               : whether to store and return attention maps
    use_softmax             : whether to normalize logits into probabilities
    '''

    pred = model(source, src_key_padding_mask=src_key_padding_mask, save_attn=save_attn) 
    
    ### COMMENTED OUT CODE: Because we always want to use gradients for attention maps [Pete]

    # if isinstance(model, ResNetSliceTrans) and save_attn: #If ResNetSliceTrans, grads required
    #     pred = model(source, src_key_padding_mask=src_key_padding_mask, save_attn=save_attn)
    # else:
    #     with torch.no_grad(): #Other model, No grads required
    #         pred = model(source, src_key_padding_mask=src_key_padding_mask, save_attn=save_attn)
    ...
    if use_softmax: # Necessary to standardize the scale before TTA average 
        ...
        #Ensures outputs are on a comparable scale
    ...
    weight = weight.view(1, 1, source.shape[2], *spatial_shape) # Reshape to [B, 1, D, H, W] 
        # D - number of slices
        # H, W - attnetion map resolution

    # Slice attention 
    weight_slice = model.get_slice_attention() # [B*D, Heads, 1] - attention over slices (each slice get a single importance value)
    ...
    #pred           : class probabilities
    #weight         : spatial attention map
    #weight_slice   : slice-wise attention map expanded spatially

#----- Prediction wrapper for ResNet models -----
...
def run_pred(model, batch, save_attn=False, use_softmax=True, use_tta=False):
    ...
    if use_tta:
        for flip_dim in [(2,), (3,), (4,), (2, 3), (2, 4), (3, 4), (2, 3, 4),]:
            ...
            # What is src_key_padding_mask: [B, D] boolean mask for padded slices????
...
if __name__ == "__main__":
    ...
    parser.add_argument('--run_folder', default='ODELIA/ResNet', type=str)
    parser.add_argument('--output_dir', default='./', type=str)
    parser.add_argument('--get_attention', action='store_true', help='Flag to get attention')
    parser.add_argument('--get_segmentation', action='store_true', help='Flag to get segmentation')
    ...
    ATTN_CLASSES = [0, 1, 2] #Add to get map from every class
    ...
    if not get_attention and not get_segmentation:
        ...
        for n, batch in enumerate(tqdm(dm.test_dataloader())):
            ...
            # Get attention maps
                ...
                cls = int(target.item())
                path_out_dir = path_out / 'attention_from_main_predict' / f'class_{cls}'
